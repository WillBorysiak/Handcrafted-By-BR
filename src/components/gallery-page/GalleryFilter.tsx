              ...
              >
                ...
                <Dialog.Panel className="relative ml-auto flex h-full  w-full max-w-xs flex-col overflow-y-auto bg-secondary py-4 pb-12 shadow-xl">
                  <div className="flex items-center justify-between px-4">
                    <h2 className="font-roman text-lg  font-bold text-primary">
                      Filters
                    </h2>
                    <button
                      type="button"
                  ...
                  <form className="mt-4 border-t border-gray-200">
                    ...
                    <ul
                      role="list"
                      className="px-2 py-3 font-roman font-medium text-primary"
                    >
                      {productTypes.map((type) => (
                        ...
                        >
                          <a className="block px-2 py-3  text-lg">
                            {type.name}
                          </a>
                        </li>
                      ))}
                    ...
                    {filters.map((section) => (
                      <Disclosure
                        as="div"
                        key={section.id}
                        className="border-t border-gray-200 px-4 py-6 font-roman"
                      >
                        {({ open }) => (
                          <>
                    ...
                    <div className="pl-2 pt-2">
                      <button
                        ...
                        onClick={() => {
                          ...
                          productTypes[0].current = true;
                          filters[0].options.forEach(
                            (item) => (item.checked = false),
                          );
                          filters[1].options.forEach(
                            (item) => (item.checked = false),
                          );
                          dispatch({
                            type: "resetFilters",
          ...
          <section aria-labelledby="products-heading" className="pt-5 pb-5">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <form className="hidden lg:block">
                ...
                <ul
                  role="list"
                  className="space-y-4 border-b border-gray-200 pb-5 text-sm  font-bold text-secondary"
                >
                  {productTypes.map((type) => (
                    <li
                      key={type.name}
                      ...
                      <>
                        <h3 className="-my-3 flow-root">
                          <Disclosure.Button className="flex w-full items-center justify-between bg-primary py-3 text-sm text-secondary hover:text-gray-500">
                            <span className="font-roman text-lg font-bold text-secondary">
                              {section.name}
                            </span>
                            <span className="ml-6 flex items-center">
                              {open ? (
                      ...
                      .filter(
                        (product: { type: string; color: []; category: [] }) =>
                          ...
                          (state.filterColor.length === 0 ||
                            state.filterColor.includes(product.color)) &&
                          (state.filterSeason.length === 0 ||
                            state.filterSeason.includes(product.category)),
                      )
                      .sort((a: { title: string }, b: { title: string }) => {
                        if (state.sort === "popular") return 0;
                        if (state.sort === "a_z" && b.title > a.title)
                          return -1;
                        if (state.sort === "z_a" && a.title > b.title)
                          return -1;
                      })}
                  />
                ) : (
